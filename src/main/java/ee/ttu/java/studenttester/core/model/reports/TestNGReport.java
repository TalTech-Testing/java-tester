package ee.ttu.java.studenttester.core.model.reports;

import com.fasterxml.jackson.annotation.JsonIgnore;
import ee.ttu.java.studenttester.core.interfaces.ReportElement;
import ee.ttu.java.studenttester.core.listeners.TestNGInterceptor;
import ee.ttu.java.studenttester.core.enums.ReportMode;
import ee.ttu.java.studenttester.core.model.tests.UnitTestContext;

import java.util.ArrayList;
import java.util.List;

public class TestNGReport implements ReportElement {

    @JsonIgnore
    public TestNGInterceptor resultListener = new TestNGInterceptor();

    @JsonIgnore
    public Integer status;

    public boolean incompleteGrade;

    public List<UnitTestContext> testContexts = new ArrayList<>();

    public int getTotalCount() {
        return testContexts.stream()
                .map(UnitTestContext::getCount)
                .mapToInt(Integer::intValue)
                .sum();
    }

    public int getTotalPassedCount() {
        return testContexts.stream()
                .map(UnitTestContext::getPassedCount)
                .mapToInt(Integer::intValue)
                .sum();
    }

    @JsonIgnore
    public int getTotalPassedWeight() {
        return testContexts.stream()
                .map(UnitTestContext::getPassedWeight)
                .mapToInt(Integer::intValue)
                .sum();
    }

    public double getTotalGrade() {
        try {
            return 1.0 * getTotalPassedWeight()
                    / testContexts.stream()
                    .map(UnitTestContext::getWeight)
                    .mapToInt(Integer::intValue)
                    .sum();
        } catch (ArithmeticException e) {
            e.printStackTrace();
            return 0;
        }
    }

    @Override
    public String toString() {
        var globalOutputBuilder = new StringBuilder()
                .append("* Unit tests *");

        testContexts.forEach(globalOutputBuilder::append);

        if (testContexts.stream().noneMatch(c -> c.configuration.mode() == ReportMode.MUTED)) {
            globalOutputBuilder.append(String.format("\nOverall grade: %.1f%%%s\n", getTotalGrade() * 100,
                    incompleteGrade ? "*\nFinal result based on only successfully compiled tests" : ""));
        }

        return globalOutputBuilder.toString();
    }
}
